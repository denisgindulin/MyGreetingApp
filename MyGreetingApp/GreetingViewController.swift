...
class GreetingViewController: UIViewController {
    ...
    //объявляю в классе GreetingViewController свойство presenter с типом GreetingPresenterProtocol (то есть создал экземпляр модели во вью контроллере, тем самым создал связь между архитектурными модулями). Неявно извлекаю опционал, поставив восклицательный знак, тем самым указываю, что свойство надо обязательно инициализировать (подготовить, наполнив значением). Позже сделаю инициализацию в методе viewDidLoad. Использовал ключевое слово private, чтобы ограничить доступ к свойству person откуда-либо вне пределов класса GreetingViewController. Использовал ключевое слово var, так как свойство person имеет опциональный тип.
