...
protocol GreetingPresenterProtocol {
    // требование протокола реализовать обязательный инициализатор. Это инициализатор протокола, поэтому здесь только имя инициализатора и параметры с указанием типа в круглых скобках, а реализации инициализатора нет (нет фигурных скобок). Реализация будет в классе, который подпишется под этот протокол.
    init(view: GreetingViewProtocol, person: Person)
    // требование протокола реализовать метод. Это метод протокола, поэтому здесь только имя метода и параметр с указанием типа в круглых скобках, а реализации метода нет (нет фигурных скобок). Реализация будет в классе, который подпишется под этот протокол.
    func showGreeting()
}

// это архитектурный модуль Presenter архитектурного паттерна MVP (Model - View - Presenter). Класс GreetingPresenter сразу, а не в расширении, подписываю под протокол GreetingPresenterProtocol, потому что реализация требований протокола - это единственное, что надо сделать внутри класса (единственное предназначение класса в отличие от класса GreetingViewController, где переопределяю методы жизненного цикла вью контроллера). Имя класса сложил из двух компонентов: 1. имя сцены (Greeting); 2. имя слоя (Presenter).
class GreetingPresenter: GreetingPresenterProtocol {
    // объявляю в классе GreetingPresenter свойство view с типом GreetingViewProtocol (тем самым создал связь между архитектурными модулями View и Presenter). Использовал ключевое слово private, чтобы ограничить доступ к свойству view откуда-либо вне пределов класса GreetingPresenter. Использовал ключевое слово unowned, чтобы разорвать цикл сильных ссылок, который мог бы образоваться из-за взаимных ссылок друг н друга классов GreetingPresenter и GreetingViewController. Чтобы указать, что ссылка не является сильной, использовал ключевое слово unowned, а не weak, так как свойство view не имеет опциональный тип. Из двух классов (GreetingPresenter и GreetingViewController) родительским классом будет GreetingViewController, дочерним - GreetingPresenter, так как GreetingViewController порождается первым. Из-за этого ключевое слово unowned использовал в классе GreetingPresenter (такое правило для разрыва цикла сильных ссылок).
